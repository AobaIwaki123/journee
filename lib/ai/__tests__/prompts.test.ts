...
export const mockItineraryAfterCollecting: ItineraryData = {
  ...
  destination: '東京',
  duration: 3,
  summary: '歴史と現代が融合する東京を満喫。浅草、渋谷、お台場を中心に観光とグルメを楽しむ旅',
  schedule: [],
  status: 'draft',
  phase: 'collecting',
  createdAt: new Date(),
...
export function testCreateNextStepPrompt() {
  ...
  const phases: ItineraryPhase[] = ['initial', 'collecting', 'skeleton', 'detailing', 'completed'];
  
  phases.forEach(phase => {
    console.log(`\n【フェーズ: ${phase}】`);
    const prompt = createNextStepPrompt(
      phase,
      phase === 'detailing' ? mockItineraryDay1Detailed : mockItineraryAfterSkeleton
    );
    console.log(prompt);
  });
...
export function testGetSystemPromptForPhase() {
  ...
  phases.forEach(phase => {
    ...
    console.log(`${phase}: ${isIncremental ? 'INCREMENTAL_SYSTEM_PROMPT' : 'SYSTEM_PROMPT'}`);
  });
...
export function demonstratePromptGeneration() {
  ...
  const nextStepPrompt = createNextStepPrompt('detailing', mockItineraryDay1Detailed);
