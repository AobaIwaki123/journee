...
export async function GET(request: NextRequest) {
  ...
  const authServiceUrl =
    process.env.AUTH_SERVICE_URL || process.env.NEXTAUTH_URL;

  // 認証プロキシモードでない場合は、通常のNextAuth認証にリダイレクト
  if (!authProxyMode) {
    const nextAuthUrl = `/api/auth/${action}?callbackUrl=${encodeURIComponent(
      callbackUrl
    )}`;
...
export async function POST(request: NextRequest) {
  ...
  console.log(
    "[Auth Proxy] Redirecting to auth service signout:",
    authUrl.toString()
  );
