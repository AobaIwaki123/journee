...
#### 3.5.3 LLM応答待ち時間のUX改善 🆕
**目的**: AIからの応答を待つ間の体験を向上させ、ユーザーに安心感と期待感を提供

##### 3.5.3.1 ローディング状態の強化
- [ ] タイピングアニメーションの実装
  - [ ] "AIが考えています..." 表示
  - [ ] ドット（...）のアニメーション
  - [ ] カスタムメッセージ（「最適なルートを探しています」等）
  - [ ] Lottieアニメーション統合（オプション）
- [ ] 骨格スクリーン（Skeleton UI）の実装
  - [ ] メッセージカードのスケルトン表示
  - [ ] しおりプレビューのスケルトン表示
  - [ ] パルスアニメーション効果
  - [ ] ストリーミング開始前の予備表示
...
##### 3.5.3.3 ストリーミング進捗の視覚化
- [ ] ストリーミング状態の明確化
  - [ ] "応答を受信中..." 表示
  - [ ] 受信バイト数の表示
  - [ ] 受信速度のインジケーター
  - [ ] リアルタイムで文字がタイプされるアニメーション
- [ ] 段階的プログレス表示
  - [ ] ステージ1: 「AIが思考中...」
  - [ ] ステージ2: 「しおりを作成中...」
  - [ ] ステージ3: 「最終調整中...」
...
##### 3.5.3.5 コンポーネント実装
- [ ] `LoadingMessage.tsx` - ローディングメッセージコンポーネント
  - [ ] タイピングアニメーション
  - [ ] カスタムメッセージ表示
  - [ ] アニメーション制御
- [ ] `SkeletonItinerary.tsx` - しおりスケルトンUI
  - [ ] ヘッダースケルトン
  - [ ] 日程スケルトン
  - [ ] スポットカードスケルトン
- [ ] `StreamingProgress.tsx` - ストリーミング進捗表示
  - [ ] プログレスバー
  - [ ] ステージ表示
  - [ ] 予想時間表示
- [ ] `WaitingTips.tsx` - 待機中のTips表示
  - [ ] Tipsローテーション
  - [ ] アニメーション効果
  - [ ] カテゴリー別Tips
...
##### 3.5.3.6 Zustand状態管理拡張
- [ ] ローディング状態の詳細化
  - [ ] `loadingStage`: 'idle' | 'thinking' | 'streaming' | 'finalizing'
  - [ ] `estimatedWaitTime`: number（秒）
  - [ ] `streamProgress`: number（0-100%）
  - [ ] `currentTip`: string
- [ ] アクションの追加
  - [ ] `setLoadingStage(stage: LoadingStage)`
  - [ ] `updateStreamProgress(progress: number)`
  - [ ] `setEstimatedWaitTime(seconds: number)`
  - [ ] `showNextTip()`

##### 3.5.3.7 旅行Tipsデータベース
- [ ] `lib/tips/travel-tips.ts` の作成
  - [ ] 旅行豆知識（100件以上）
  - [ ] アプリの使い方
  - [ ] 旅行計画のコツ
  - [ ] カテゴリー別分類
...
**関連コンポーネント**:
- `components/chat/MessageList.tsx` - ローディングメッセージ表示
- `components/chat/MessageInput.tsx` - 送信後のフィードバック
- `components/itinerary/ItineraryPreview.tsx` - スケルトンUI
- `components/ui/LoadingSpinner.tsx` - 既存スピナーの拡張
- `lib/store/useStore.ts` - ローディング状態管理

**実装優先度**:
1. 🔴 **高**: タイピングアニメーション、スケルトンUI
2. 🟡 **中**: 待機中のTips表示、プログレスバー
3. 🟢 **低**: ミニゲーム、Lottieアニメーション

**詳細ドキュメント**: [docs/PHASE3.5.3_LOADING_UX.md](./docs/PHASE3.5.3_LOADING_UX.md)（作成予定）
...
- 🔄 **Phase 3.5.2**: UI/UX改善（AIモデル選択トグル完了、テキストハイライト進行中）
