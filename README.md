...
#### 4.8 必要情報チェックリストUI 🆕
**目的**: ユーザーが旅のしおり作成に必要な情報を一目で把握し、入力を促すチェックリストUIを実装

##### 4.8.1 UIコンポーネント設計
- [ ] `RequirementChecklist.tsx` コンポーネントの作成
  - [ ] アコーディオンUI実装（開閉可能）
  - [ ] 必要情報のチェックリスト表示
    - [ ] 行き先（必須）
    - [ ] 旅行期間（必須）
    ...
    - [ ] 移動手段の好み（オプション）
    - [ ] 宿泊施設の好み（オプション）
  - [ ] チェック状態の視覚化（✓/○アイコン、プログレスバー）
  - [ ] 充足率の表示（例: 75%完了）
  - [ ] 未入力項目のハイライト表示

##### 4.8.2 レイアウト・配置
- [ ] **配置位置**: 進捗UI（PlanningProgress）直下、しおりコンポーネント領域内
- [ ] **サイズ**: しおりコンポーネントの幅50% × 高さ50%
- [ ] **位置**: しおりコンポーネントの右上に配置
- [ ] **レイヤー**: しおりコンポーネントの上にオーバーレイ（z-indexで制御）
- [ ] **背景**: 半透明の白背景（backdrop-blur効果）でしおり内容が透けて見える
- [ ] **スクロール**: しおりコンテンツは背後でスクロール可能
- [ ] **アニメーション**: 
  - [ ] アコーディオン開閉時のスムーズなアニメーション
  - [ ] 表示/非表示のフェードイン・フェードアウト
  - [ ] ホバー時のエフェクト

##### 4.8.3 情報抽出ロジック
- [ ] チャット履歴からの情報自動抽出
  - [ ] `lib/requirements/extractors.ts` の活用
  - [ ] 正規表現ベースの抽出（日付、数値、地名）
  - [ ] AIベースの抽出（Phase 4.8全体機能との連携）
- [ ] リアルタイム更新
  - [ ] メッセージ送信ごとに情報を再抽出
  - [ ] チェックリスト状態を自動更新
  - [ ] 充足率の再計算
...
##### 4.8.4 状態管理
- [ ] Zustand storeの拡張
  - [ ] `requirementChecklistVisible: boolean` - チェックリスト表示状態
  - [ ] `extractedRequirements: ExtractedRequirements` - 抽出された情報
  - [ ] `requirementCompletionRate: number` - 充足率（0-100）
  - [ ] `toggleRequirementChecklist()` - 表示切り替えアクション
  - [ ] `updateExtractedRequirements()` - 情報更新アクション
- [ ] LocalStorageへの永続化
  - [ ] チェックリスト開閉状態の保存
  - [ ] 抽出済み情報のキャッシュ
...
- [ ] 未入力項目のクリック
  - [ ] クリック時にチャット入力欄にフォーカス
  - [ ] 入力例をプレースホルダーに表示
  - [ ] AIへのプロンプト提案（例: 「行き先を教えてください」）
- [ ] 充足率に応じた次のステップ提案
  - [ ] 80%以上: 「骨組み作成」ボタンを強調表示
  - [ ] 50-79%: 「もう少し情報を追加すると、より良いしおりを作成できます」
  - [ ] 50%未満: 「基本情報を入力してください」

##### 4.8.6 レスポンシブ対応
- [ ] デスクトップ（1024px以上）
  - [ ] しおり領域の右上に配置（幅50% × 高さ50%）
  - [ ] オーバーレイ表示
- [ ] タブレット（768px - 1023px）
  - [ ] しおり領域の上部に配置（幅100% × 高さ40%）
...
##### 4.8.7 視覚デザイン
- [ ] カラースキーム
  - [ ] 背景: `bg-white/90 backdrop-blur-md`（半透明白 + ぼかし）
  - [ ] ボーダー: `border border-gray-200/50`
  - [ ] 影: `shadow-xl`（浮いているように見せる）
  - [ ] 完了項目: `text-green-600`（緑）
  - [ ] 未完了項目: `text-gray-400`（グレー）
- [ ] アイコン
  - [ ] 完了: `CheckCircle2`（lucide-react）
  - [ ] 未完了: `Circle`（lucide-react）
  - [ ] 必須: `AlertCircle`（lucide-react、赤色）
  - [ ] 推奨: `Info`（lucide-react、青色）
  - [ ] オプション: `Sparkles`（lucide-react、紫色）
- [ ] プログレスバー
  - [ ] 水平プログレスバー（0-100%）
  - [ ] グラデーション: `bg-gradient-to-r from-blue-500 to-green-500`
  - [ ] アニメーション: 充足率変化時にスムーズに伸縮

##### 4.8.8 Phase 4.8全体機能との連携
- [ ] フェーズ遷移判定への統合
  - [ ] 充足率80%以上で「骨組み作成」を推奨
  - [ ] QuickActionsコンポーネントとの連携
- [ ] `lib/requirements/checklist-utils.ts` との連携
  - [ ] `checkRequirementCompleteness()` 関数の活用
  - [ ] `getRecommendedNextStep()` 関数の活用

##### 4.8.9 アクセシビリティ
- [ ] ARIA属性の追加
  - [ ] `role="region"` - チェックリスト領域
  - [ ] `aria-expanded` - アコーディオン開閉状態
  - [ ] `aria-labelledby` - セクションラベル
  - [ ] `aria-describedby` - 説明文
- [ ] キーボード操作
  - [ ] Tab: フォーカス移動
  - [ ] Enter/Space: アコーディオン開閉
  - [ ] Escape: チェックリスト非表示
- [ ] スクリーンリーダー対応
  - [ ] チェック状態の読み上げ
  - [ ] 充足率の読み上げ
...
- [ ] メモ化
  ...
  - [ ] `useMemo` で充足率計算をキャッシュ
  - [ ] `useCallback` でイベントハンドラー最適化
- [ ] 遅延レンダリング
  - [ ] アコーディオン開閉時のみ内容をレンダリング
  - [ ] 非表示時はDOMから削除
- [ ] デバウンス
  - [ ] 情報抽出処理を500msデバウンス（頻繁な再計算を防止）

**実装の流れ**:
1. `RequirementChecklist.tsx` コンポーネントの基本構造作成
2. アコーディオンUIの実装（開閉機能）
3. チェックリスト項目の表示とスタイリング
4. 情報抽出ロジックとの連携
5. Zustand状態管理との統合
6. レイアウト・配置の調整（オーバーレイ、位置、サイズ）
7. レスポンシブ対応
8. アクセシビリティ対応
9. パフォーマンス最適化
10. テスト・デバッグ
...
**期待される効果**:
- ✨ ユーザーが必要な情報を一目で把握できる
- ✨ 入力忘れを防ぎ、より完成度の高いしおりを作成できる
- ✨ 次のステップへの進行がスムーズになる
- ✨ チャット入力の迷いを減らし、UXが向上
- ✨ フェーズ移動の半自動化（Phase 4.8全体）を視覚的にサポート
...
#### 7.1 リサイザー機能実装 ✅ **完了** (2025-10-08)
- [x] `ResizablePanel.tsx` コンポーネントの作成
  - [x] ドラッグ可能なリサイザーバーの実装
  - [x] マウスイベント（`mousedown`, `mousemove`, `mouseup`）のハンドリング
  - [x] タッチイベント対応（モバイル互換性）
  - [x] 最小幅・最大幅の制限（チャット: 30-70%、しおり: 30-70%）
- [x] `ResizableLayout.tsx` コンポーネントの作成
  - [x] Zustandストアとの連携
  - [x] 動的幅レイアウト
- [x] `app/page.tsx` の更新
  - [x] 固定幅（`w-2/5`, `w-3/5`）から動的幅に変更
  - [x] リサイザーコンポーネントの統合
  - [x] リサイズ状態の管理（Zustand）
- [x] LocalStorage による幅の永続化
  - [x] ユーザーが設定した幅を保存
  - [x] ページリロード時に復元
- [x] リサイザーバーのUI実装
  - [x] ホバー時のビジュアルフィードバック
  - [x] ドラッグ中のカーソル変更（`cursor: col-resize`）
  - [x] 縦の区切り線デザイン
  - [x] ツールチップ表示
  - [x] アクセシビリティ対応

**実装結果**:
- ✅ チャットとしおりの幅を30-70%の範囲で自由に調整可能
- ✅ スムーズなドラッグ操作（マウス・タッチ対応）
- ✅ ホバー・ドラッグ時の視覚的フィードバック
- ✅ LocalStorageによる設定の永続化
- ✅ アクセシビリティ対応（role, aria-label, tabIndex）

**詳細**: [docs/PHASE7_1_RESIZABLE_PANEL.md](./docs/PHASE7_1_RESIZABLE_PANEL.md)
...
**原因分析**:
- `MapView.tsx` は `spot.location?.lat && spot.location?.lng` でフィルタリング
- モックデータ（`lib/mock-data/itineraries.ts`）の全てのスポットに `location` プロパティが含まれていない
- 結果: `spots.length === 0` となり、「位置情報が設定されたスポットがありません」と表示される

**影響範囲**:
- ✅ `MapView.tsx` コンポーネント自体は正常動作（location付きスポットがあれば表示される）
- ❌ モックデータから読み込んだしおりは地図表示不可
- ✅ AIチャットで生成された新規しおりはlocation情報が含まれる場合あり（プロンプト次第）

**対応方針**:
1. モックデータにlocation情報を追加
2. 主要な観光スポットの緯度・経度を設定
3. AIプロンプトでlocation生成を強化（Phase 4で対応予定）

**修正内容**:
- [ ] `lib/mock-data/itineraries.ts` の更新
  - [ ] 東京3日間の旅（浅草寺、スカイツリー等）
  - [ ] 京都紅葉巡り（渡月橋等）
  - [ ] 北海道グルメツアー（時計台、すすきの等）
  - [ ] 大阪食い倒れツアー（道頓堀等）
...
**回避策（現時点）**:
- AIチャットで「地図に表示してほしい」と明示的にリクエストすると、AIがlocation情報を含むしおりを生成する

**関連Phase**: Phase 5.1.3（地図統合）、Phase 4（プロンプト改善）
...
- ✅ **Phase 5.1.3**: 高度な機能（ドラッグ&ドロップ、Undo/Redo、テンプレートシステム、パフォーマンス最適化）
  - ✅ **地図機能**: Google Maps統合（マーカー表示、ルート描画、情報ウィンドウ）※モックデータ対応は進行中（BUG-004）
...
- ✅ **Phase 7.1**: リサイザー機能実装（ドラッグ&ドロップ、LocalStorage永続化、30-70%制限）
...
- **Phase 7.2** - モバイル対応（タブ切り替え、タッチ操作、レスポンシブデザイン）

**最終更新**: 2025-10-08 (Phase 7.1完了)
...
- [Phase 7.1 実装完了レポート](./docs/PHASE7_1_RESIZABLE_PANEL.md)
