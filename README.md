...
- [ ] `RequirementsChecklist` コンポーネント
  ...
  - [ ] 充足率プログレスバー
  - [ ] 不足情報の入力促進メッセージ
- [ ] `PlanningProgress` との統合
  - [ ] チェックリストをプログレスバー下部に表示
...
##### 4.9.1 並列処理APIの設計
- [ ] バッチリクエスト型定義
  - [ ] `BatchDayDetailRequest` 型（複数日の詳細化リクエスト）
  - [ ] `DayDetailTask` 型（各日の詳細化タスク）
  - [ ] `BatchDayDetailResponse` 型（並列処理の結果）
- [ ] 新しいAPIエンドポイント
  - [ ] `POST /api/chat/batch-detail-days`
  - [ ] 複数日の詳細化を同時にリクエスト
  - [ ] 各日ごとにGemini APIを呼び出し
